...
class MarketDataValidator:
    ...
    def compare(self, primary: list[Candle], secondary: list[Candle], tolerance: float = 0.001) -> list[DataAnomaly]:
        other = {item.timestamp: item for item in secondary}
        anomalies: list[DataAnomaly] = []
        for candle in primary:
            peer = other.get(candle.timestamp)
            if peer and abs(candle.close - peer.close) / max(candle.close, peer.close) > tolerance:
                anomalies.append(DataAnomaly(type=AnomalyType.PROVIDER_INCONSISTENCY, timestamp=candle.timestamp, severity=2, detail=f"{candle.provider} close differs from {peer.provider}"))
        return anomalies
