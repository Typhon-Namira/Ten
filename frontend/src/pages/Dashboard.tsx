...
export function Dashboard() {
  const { signals, engines, market, loading, error, refresh } = useDashboard()
  ...
  return <div className="page">
    ...
    <section className="metrics" id="overview">
      ...
      <MetricCard label="Active bias" value={latest?.direction.toUpperCase() ?? 'NEUTRAL'} detail={latest ? `${latest.timeframe} composite scenario` : 'Awaiting engine confluence'} icon={<SignalIcon size={18} />} />
      <MetricCard label="Confidence" value={latest ? `${Math.round(latest.confidence * 100)}%` : '—'} detail="Deterministic weighted score" icon={<Activity size={18} />} />
