...
function CalibrationPanel({ data }: { data: ReturnType<typeof useAIDashboardData> }) {
  ...
  return <section className="ai-card ai-card--wide">
    ...
    {!calibration ? <EmptyState title="Calibration unavailable" detail="No backend calibration report is available." /> : <>
...
function ReadinessDetail({ data }: { data: ReturnType<typeof useAIDashboardData> }) {
  ...
  return <section className="ai-card ai-card--wide">
    ...
    {!readiness ? <EmptyState title="Readiness not evaluated" detail="TEN has not persisted a production-readiness report." /> : <div className="readiness-checks">
...
export function AIDashboard({ view = 'overview' }: { view?: DashboardView }) {
  ...
  return <div className="ai-page">
    ...
    {data.loading && !data.reasoning && !data.intelligence ? <LoadingSkeleton rows={6} /> : <>
      {view === 'overview' && <>
        <FinalDecisionHero intelligence={data.intelligence} reasoning={data.reasoning} />
        <DecisionPipeline intelligence={data.intelligence} quant={data.quant} reasoning={data.reasoning} />
        <div className="ai-card-grid">
          <MarketStateSummary data={data.intelligence} />
          <QuantForecastSummary forecast={data.quant} calibration={data.calibration} />
          <AIReasoningSummary data={data.reasoning} />
          <GuardrailSummary data={data.reasoning} />
        </div>
        <ActiveSignalMonitor data={data.reasoning} market={data.intelligence} />
        <ValidationSummary data={data.reasoning} />
        <OperationalHealth data={data.reasoning} stale={data.stale} />
      </>}
      {view === 'signals' && <>
        <FinalDecisionHero intelligence={data.intelligence} reasoning={data.reasoning} />
        <ActiveSignalMonitor data={data.reasoning} market={data.intelligence} />
        <div className="ai-card-grid"><GuardrailSummary data={data.reasoning} /><AIReasoningSummary data={data.reasoning} /></div>
      </>}
      {view === 'performance' && <>
        <ValidationSummary data={data.reasoning} />
